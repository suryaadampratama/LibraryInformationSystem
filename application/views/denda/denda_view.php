				...
				<div class="col-sm-4">
					<div class="box box-primary">
						<div class="box-header with-border">
							...
							<h4> Edit Fine Price</h4>
							<?php }else{?>
							<h4> Add Fine Price</h4>
							<?php }?>
						</div>
						<!-- /.box-header -->
						<div class="box-body">
							...
							<form method="post" action="<?= base_url('transaksi/dendaproses');?>">
								...
								<div class="form-group">
									...
									<select class="form-control" name="status">
										<option <?php if($den->stat == 'Aktif'){echo 'selected';}?>>Active</option>
										<option <?php if($den->stat == 'Tidak Aktif'){echo 'selected';}?>>Not active</option>
									</select>
								</div>
								<br/>
								<input type="hidden" name="edit" value="<?=$den->id_biaya_denda;?>">
								<button type="submit" class="btn btn-primary"><i class="fa fa-edit"></i> Edit Fine Price</button>
							</form>
							<?php }else{?>
							...
							<form method="post" action="<?= base_url('transaksi/dendaproses');?>">
								...
								<button type="submit" class="btn btn-primary"> <i class="fa fa-plus"></i> Add Fine Price</button>
							</form>
							<?php }?>
						</div>
					</div>
